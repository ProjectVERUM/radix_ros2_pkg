![radix_ros_node_img](./docs/assets/imgs/radix_ros.drawio.png)
...
```
3. Go back to your `ws_radix` and build
```bash
cd ~/ws_radix && colcon build --packages-select radix_ros --symlink-install
```
4. Source your workspace setub.bash file

```bash
source ~/ws_radix/install/setup.bash
```
...
```bash
ros2 launch radix_ros radix_mapping.launch.xml
```

There is another launch file that allows you to modify all the available parameters within the params/radix_params.yaml and launch the radix_server_node along RViz2.

```bash
ros2 launch radix_ros radix_mapping.launch.py
```

<p align="center">
  <img src="./docs/assets/imgs/nice_vis.gif" alt="Sublime's custom gif"/>
</p>

# 3. ROS 2 node API

<p align="center">
  <img src="./docs/assets/imgs/radix_ros_node.drawio.png" alt="Sublime's custom image"/>
</p>
...
`radix_point_cloud_centers (sensor_msgs/msg/PointCloud2)`
