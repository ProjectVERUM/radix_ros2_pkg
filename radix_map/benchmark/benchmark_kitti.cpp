...
#include "radix_map/pcl_utils.hpp"
#include "radix_map/probabilistic_map.hpp"
...
Eigen::Isometry3d ReadCalibration(const std::string& calibration_file) {
  ...
  while (std::getline(input, line)) {
    ...
    ss >> header >> rot(0, 0) >> rot(0, 1) >> rot(0, 2) >> pos(0) >>
        rot(1, 0) >> rot(1, 1) >> rot(1, 2) >> pos(1) >> rot(2, 0) >>
...
void ReadPointcloud(const std::string& cloud_file,
                    const Eigen::Isometry3d& transform, PointCloudT& points) {
...
int main(int argc, char** argv) {
  ...
  options.add_options()("clouds", "Pointcloud folder path",
      ...
      "skip_octree", "Do not compute the octree",
  ...
  for (size_t count = 0; count < cloud_filenames.size(); count++) {
    ...
    std::cout << "[" << filename << "] bonxai time: " << diff << " ms"
              << std::endl;
  ...
  if (!skip_octree) {
    for (size_t count = 0; count < cloud_filenames.size(); count++) {
      ...
      ReadPointcloud<octomap::Pointcloud, float>(filename, transform,
                                                 pointcloud);
